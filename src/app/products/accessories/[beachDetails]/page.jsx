...
const BeachDetailsPage = async ({ params }) => {
  const { beachDetails } = await params;
  const res = await fetch("https://beach-product.onrender.com/beach");
  const data = await res.json();
  const beach = data.find((be) => be.id == beachDetails);
  const { name, brand, price, rating, image, description } = beach;

  return (
    <div className="max-w-[620px] bg-white border border-gray-200 p-8 rounded-xl space-y-1 shadow-2xl mt-16 mb-16 mx-auto space-y-2">
